...
const StudentNocAppliedStatusTable =async ({columnsName,data}) => {
//  console.log(data)
  return (
    <div>
 ...
 <TableRow key={id}>
           ...
           <TableCell>{e?.comment ? e?.comment : <p>No Comments.</p>}</TableCell>
           <TableCell>{e?.allowededit ? <Link href={`edit-request/${e?.id}`}><Button size='icon'><FilePenLine /></Button></Link> : <Button className='cursor-not-allowed' size="icon" disabled><FilePenLine /></Button> }</TableCell>
...
export default StudentNocAppliedStatusTable
